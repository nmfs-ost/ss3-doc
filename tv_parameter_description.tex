...
For example, if two parameters were specified to have environmental linkages in the mortality-growth (MG) parameter section, below the MG parameters would be two parameter lines (when not auto-generating these lines), which is an environmental linkage parameter for each time-varying base parameter:
