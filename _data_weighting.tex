...
\begin{figure}[ht]
	...
	\alt{Plot created from the r4ss function SS\_plots() when tuning using the McAllister-Ianelli method has been implemented. The y-axis is effective sample size and the x-axis is observed sample size. A horizontal green dotted line shows the harmonic mean, a vertical green dotted line shows the arithmetic mean, and a black diagonal line is a one-to-one line.}
...
\begin{figure}[ht]
	...
	\alt{A plot created by r4ss using tuning according to the Francis method. The y-axis is the mean length and the x-axis is the Year from the MexCal S1 NSP fleet (whole catch).}
	\label{fig:francis}
	%\figurename{name}
\end{figure}
