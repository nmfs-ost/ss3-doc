...
\end{center}
	...
	\pagebreak
	\subsubsection{TimeVary Parameter Controls}
	In SS3.30, several changes are introduced to the implementation of timevarying parameters.
\begin{itemize}
  \item	timevarying parameters for biology, spawner-recruitment, catchability, and selectivity are implemented using the same approach and share code.
  \item the block feature that allowed input of one block parameter line and replication of that line by SS as often as needed has been replaced.  Now there is a complete timevary parameter autogeneration capability.
  \item The logistic bound constraint is no longer implemented due to the challenges it created to interpreting parameter values.  Instead, the autogenerate feature now creates bounds on timevary parameters for blocks such that the combination of a bounded  value of the timevary parameter and the base parameter will not violate the base parameter bounds.
  \item For more information on the implementation of timevary parameters, see that section at:  xxxx
\end{itemize}	
	
\begin{center}
	\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
     \multicolumn{2}{l}{} & \multicolumn{1}{l}{controls for all timevary  parameters}\\
     \hline
		1 & & \multirow{1}{6cm}[-0.1cm]{\parbox{12cm}{env/block/dev adjust method for all time-vary parms (1=warn relative to base parm bounds; 3=no bound check)}}
		\\ \\ \\
		& 0 1 0 0 2  & \multirow{1}{6cm}[-0.1cm]{\parbox{12cm}{Five values control autogen for:  1-biology, 2-spawnrecr, 3-catchability, 4-tag (future), 5-selectivity.  \\ The accepted values are:\\
				0 to autogenerate all time-varying parameters;\\
				1 to read each time-varying parameter line;\\
			    2 to read each line and autogen if read value for parm min = -12345}}
	\end{longtable}
\end{center}
...
\begin{center}
	...
	\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	   ...
	   0.20 & & Exponential decay for growth above maximum age (fixed at 0.20 in 3.24; should approximate initial Z).   Setting value to -999 will replicate the simpler calculation done in SS3.24.\\
	   \hline
...
\begin{center}
	...
	\end{longtable}
\end{center}
The number of parameters needed by each relationship is stored internally.  In SS3.24 and before, only a short parameter line was used for the spawner-recruitment section. SS v3.30 now requires long parameter lines in the spawner-recruitment section because it now uses the same time-varying parameter approach as the biology and selectivity parameters.  This generic time-varying approach replaces the SR envlink concept in SS3.24.  Also the R1 offset was effectively a block to implement a regime shift for the initial equilibrium year.  Now in 3.30, the R1 offset parameter is replaced by a parameter termed "regime shift".  This regime shift parameter can have blocks, environmental links or random devs. 
\begin{center}
	\begin{longtable}{p{1cm} p{3cm} p{11cm}}
...
\begin{center}
	\begin{longtable}{p{1.1cm} p{1.1cm} p{1.2cm} p{1.2cm} p{1.5cm} p{1.4cm} p{1.5cm} p{4.2cm}}
		...
		\multicolumn{8}{l}{A long parameter line is expected for each link parameter (i.e. Q) and for the extra SD parameter.}\\
		\hline
	...
	\end{longtable}
\end{center}
Then because the Q base parameter specifies that it is time-varying by the annual dev method, short parameter lines to specify the specifications of the dev vector come after all the base Q parameters.
