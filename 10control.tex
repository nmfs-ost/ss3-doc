...
\begin{description}
	...
	\begin{equation}
		vec(\epsilon) \sim MVN(0,\sigma_s^2R_{total})
	\end{equation}
	where $\epsilon$ is the two-dimensional deviation matrix and $\sigma_s^2R_{total}$ is the covariance matrix for the 2D AR1 process. More specifically, $\sigma_s^2$ quantifies the variance in selectivity deviations and $R_{total}$ is equal to the knonecker product ($\otimes$) of the two correlation matrices for the among-age and among-year AR(1) processes:
	\begin{equation}
		R_{total}=R\otimes\tilde{R}
	\end{equation}
\end{description}
