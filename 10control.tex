...
\begin{center}
	\begin{tabular}{p{2cm} p{3cm} p{10cm}}
		...
		%\hline
		 & & \\
		\multicolumn{3}{l}{\parbox{16cm}{Note that relative to SS3.24,  the order of Prior stdev and Prior type have been switched and the prior type options have been renumbered.
...
\subsubsection{Miscellaneous setups:}
The top section of the control file has various controls for growth, movement, morphs.  The order in which they appear in the control file has grown over time rather opportunistically, so it may not appear particularly logical at this time.  When the same information is entered via the GUI, it is organized more logically and then written in this form to the text control file.
...
\begin{center}
	...
	\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
		...
		3 & & Number of platoons within a growth pattern to allow exploration of size-dependent survivorship. A value of 1 will not create additional platoons.  Odd-numbered values of 3 - 5 will break the overall cohort into that number of platoons.  More platoons slows model execution, so values above 5 not advised.\\
...
\begin{center}
	...
	\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	   ...
	   & 4 & \multirow{1}{12cm}[-0.1cm]{Number of breakpoints.  A value of 2 would correspond to the M pattern as defined in SS V2.  Then read a vector of ages for these breakpoints. Later, per sex x GP, read N parameters for the natural mortality at each breakpoint.}
	   \\
	   \\
	   \\
	   \\
...
\begin{center}
	...
	\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	   ...
	   0.20 & & Exponential decay for growth above maximum age (fixed at 0.20 in 3.24; should approximate initial Z).  In SS v.3.30, this now uses the von Bertalanffy growth out to 3*nages and to decay the numbers at age by exp(-value set here).\\
	   \hline
	   ...
	   -999 & & Exponential decay for growth above maximum age (fixed at 0.2 in 3.24; should approx initial Z).\\
	   \hline
...
\hline
	...
	\end{longtable}
\end{center}
\paragraph{Parameter Offset Method}
\begin{center}
...
\begin{center}
	\begin{longtable}{ p{0.7cm} p{0.7cm} p{0.7cm}  p{1cm}  p{1.4cm}  p{1cm}  p{8cm}  }
		...
		\multicolumn{7}{l}{Required Cohort growth deviation specification} \\
		...
		\multicolumn{7}{l}{COND: Only if multiple age-length keys are defined} \\
		-99  & 99   &  1   & 0    & ... & 0 & \#AgeKeyParm1 \\
		-99  & 99   &  0.288   & 0    & ... & 0 & \#AgeKeyParm2 \\
		-99  & 99   &  0.715   & 0    & ... & 0 & \#AgeKeyParm3 \\	
		\multicolumn{7}{l}{Required sex ratio specification} \\
		0.001 & 0.999 & 0.5 & 0.5    & ... & 0 & \#FracFemale\_GP\_1 \\		
		0.001 & 0.999 & 0.5 & 0.5    & ... & 0 & \#FracFemale\_GP\_2 \\	
		\hline
	\end{longtable}
\end{center}
...
\begin{center}
	\begin{longtable}{p{1.1cm} p{1.1cm} p{1.2cm} p{1.2cm} p{1.5cm} p{1.4cm} p{1.5cm} p{4.2cm}}
		...
		\multicolumn{8}{l}{A long parameter line is expected and the above example would be specified by:}\\
		\hline
...
\begin{center}
	\begin{longtable}{p{1cm} p{1cm} p{1cm} p{1.2cm} p{1.2cm} p{1.1cm} p{1.5cm} p{5.2cm}}
		...
		\hline
		0.001 & 2    & 0.079 & 0.079 & 0.5   & 4       & -5   & \#LnQ\_base\_Survey\_dev\_se\\
		-0.99 & 0.99 & 0.00  & 0.00  & 0.5   & 4       & -6   &  \#LnQ\_base\_Survey\_dev\_se\\
		\hline
	\end{longtable}
\end{center}
...
\hypertarget{TVpara}{}; \hypertarget{time-vary}{}
