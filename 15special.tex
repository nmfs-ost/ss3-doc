...
\begin{itemize}
	\item \textbf{If using the N_breakpoints method:} Multiply the natural mortality age breakpoints and growth Amin (referred to in the control file as "Age post-settlement\_for\_L1") and Amax (referred to as "Growth\_Age\_for\_L2") values by \(\frac{1}{\text{season duration}}\).
	...
	\item Parameters:
		...
		\item Decrease the $R_0$ parameter starting value because it is now the average number of recruitments per quarter year.
