...
%Quantities are calculated at the following times:	
%\begin{itemize}
...
%	\item Survey timing is now observation-specific and specified in units of months.fraction (e.g., Apr 15 = 4.5).
...
\begin{center}
	\begin{tabular}{p{2.25cm} p{2.65cm} p{2.25cm} p{2.5cm} p{2.5cm} p{2cm} }
		...
		\hline
		a: 			   & b:  			  & c:    & d:     & e:    & f: \Tstrut\\
		 Fleet Index   & Include in MSY   & Fmult & F or First Year & F or Last Year & Not used \Bstrut\\			
		\hline
...
\begin{center}
	\begin{tabular}{p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm} p{1.1cm} p{3.5cm}}
		...
		\hline
		& Year & Month & Type & Part & Nsamp & \multicolumn{2}{l}{Data Vector} \Tstrut\Bstrut\\
		\hline
...
\myparagraph{Note}
	\begin{itemize}
		...
		\item The "partition" flag is included here in the data, but cannot be used because the expected value is calculated before the catch is partitioned into discard and retained components. Currently, this value must be set to 0 (combined discard and retained fish).
