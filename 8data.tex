...
\subsection{Time Units}
	\begin{itemize}
		\item Spawning:
		\begin{itemize}
			\item Restricted to happening once per year at a specified date (in real months).
		\end{itemize}
		
		\item Recruitment:   
		\begin{itemize}
			\item Occurs at specified recruitment events that occur at user-specified dates (in real months).
			\item There can be one to many recruitment events; each producing a platoon as a portion of the total recruitment.
...
\begin{itemize}
	\item There is no retained catch amount to match and hence, the F for bycatch only fleets must be implement a continuous F method  (F\_method = 2).
...
The options for units for input data are:	
	\begin{itemize}
		...
		\end{itemize}
		\item \hypertarget{SpecialSurvey}{>=30} special survey types.  These options bypass the calculation of survey selectivity so no selectivity parameters should be entered and especially not estimated.  The expected values for these types are:
		\begin{itemize}
