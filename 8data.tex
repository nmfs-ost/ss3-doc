...
\myparagraph{Excluding Data}	
	\begin{itemize}
		...
		\item If the value of fleet is negative, then the observation is processed and its expected value and log likelihood are calculated, but this log likelihood is not included in the total log likelihood. This feature allows the user to see the fit to a provisional observation without having that observation affect the model.
	\end{itemize}
...
\myparagraph{Note}
	\begin{itemize}
		...
		\item A number of fish value of 0 will cause mean size value to be ignored in fitting.
		...
		\item The "Ignore" column is not used (set aside for future options) but still needs to have default values in that column. 
...
\myparagraph{Note}
	\begin{itemize}
		\item The release data must be enter in tag group order.
