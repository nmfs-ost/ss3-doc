...
The full parameter line (14 in length) syntax for the mortality-growth, spawn-recruitment, selectivity, and Q sections provides additional controls to give the parameter time-varying properties.  Short parameter lines, the first 7 elements, are used for any time-varying parameter.  Additional information regard time-varying parameters is available in the \hyperlink{TVpara}{Using Time-Varying Parameters} section.


\subsection{Terminology}
Where the term COND appears in the value column of this documentation (it does not actually appear in the control file), it indicates that the following section is omitted except under certain conditions, or that the factors included in the following section depend upon certain conditions. In most cases, the description in the Definition column is the same as the label output to the control.ss\_new file.
...
\begin{center}
	\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
		...
		0 & & 0 = do not read the wtatage.ss file; \Tstrut\\
		  & & 1 = read the wtatage.ss file, also read and use the growth parameters; and \\
		...
		%\hline
		\pagebreak
		2 & & Number (N) of growth patterns (GP), also referred to as morphs:\Tstrut\\
		& & These are collections of fish with unique biological characteristics (growth, mortality, weight-length, reproduction). The GP x Sex x Settlement Events constitute unique morphs that are tracked in SS. They are assigned these characteristics at birth and retain them throughout their lifetime. At recruitment, morph members are distributed across areas (if any) and they retain their biological characteristics even if they move to another area in which a different cohort with different biological characteristics might predominate.  For example, one could assign a fast-growing morph to recruit predominately in a southern areas and a slow-growing morph to a northern area. The natural mortality and growth parameters are specified for each growth pattern in the mortality-growth parameter section in the order of females growth pattern 1 to growth pattern N followed by males growth pattern 1 to growth pattern N.\Bstrut\\

		\hline
		3 & & Number of platoons within a growth pattern/morph:\Tstrut\\
...
\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	...
	\hline
	0 \Tstrut & & Year x area x settlement event interaction (only for recruitment distribution method = 1). \Bstrut\\
...
\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	...
	\pagebreak
	\Tstrut & 1 1 1 2 4 10 & \multicolumn{1}{l}{\multirow{5}{6cm}{\parbox{12cm}{The four requested movement definitions appear here.  Each definition specifies: season, growth pattern, source area, destination age1, age2. The rate of movement will be controlled by the movement parameters later.  Here the age1 and age2 controls specify the range over which the movement parameters are interpolated with movement constant below age1 and above age2.}}}\\
	& 1 1 2 1 4 10 &  \Tstrut\\
	& 1 2 1 2 4 10 &  \\
	& 1 2 2 1 4 10 &  \Bstrut\\
	\\ 
...
\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	...
	\multicolumn{2}{l}{0 0 0 0 0} & Autogeneration: five values control auto-generation for: 1-biology, 2-spawn-recruitment, 3-catchability, 4-tag (future), and 5-selectivity. The accepted values are:\\
	&           & 0 = auto-generate all time-varying parameters;\\
	& 			& 1 = read each time-varying parameter line; and\\
	&			& 2 = read each line and auto-generate if read value for parameter min = -12345. \Bstrut\\
	\hline
\end{longtable}
...
\begin{longtable}{p{0.5cm} p{2cm} p{12cm}}
	...
	1 & & Growth Model: \Tstrut\\
	  ...
	  & & 5 = age specific K. Set base K as K for nages and work backwards and the age-specific K = base K * multiplier; and\\
...
\begin{longtable}{p{0.5cm} p{2cm} p{13cm}}
	...
	\multicolumn{2}{r}{0 0.05 0.10 ...} & Vector of length-specific maturity or fecundity.  One row of length equal to the number of population length bins (defined in the data file) for each female growth pattern. \Bstrut\\
	\hline
...
\begin{longtable}{p{1cm} p{2.5cm} p{10cm}}
	...
	& CV old & Variability for size at age >= Amax for females, growth pattern 1. For intermediate ages, do a linear interpolation of CV on means size-at-age.  Note that the units for CV will depend on the CV pattern and the value of mortality-growht parameter as offset. The CV value cannot vary over time.\Bstrut\\
	\hline
	...
	& Mat-50 & Maturity logistic inflection (in cm or years) where female maturity-at-length (or age) is a logistic function: $M_{l} = 1/(1+exp(\alpha*(l_{a} - \beta)))$. The $\alpha$ is the slope, $l_{a}$ is the size-at-age, and $\beta$ is the inflection of the maturity curve. \\ 
	...
	\multicolumn{2}{l}{COND: Recruitment Distribution Method = 2 or 3} \Tstrut & 3 parameters lines defining recruitment distribution.\\
	...
	\multicolumn{2}{l}{Recruitment Dist. Season} & Recruitment apportionment by season, if multiple seasons, multiple entries required\Bstrut\\
	\hline
...
SS allows a short-cut for males to use the same parameter values as female fish for natural mortality, length minimum (L1), maximum length (L2), coefficient at younger ages (CV1), coefficient at older ages (CV2)and the growth coefficient (K).  If the INIT parameter value for males is set equal to 0.0 and the phase set to negative, not estimated, each of these male parameters will use the  corresponding female parameter value for the males. 
...
Seasonal effects are available for weight-length parameters and for the growth K. Seasonality is not needed for the maturity and fecundity parameters because spawning is only defined to occur in one season.  Seasonality on the minimum length parameter (L1) may be implemented at a later date. The seasonal parameter values adjust the base parameter value for that season.
\begin{equation}
...
\begin{longtable}{p{1cm} p{3cm} p{11cm}}
	...
	3 & Spawner-            & The options are: \Tstrut\\
      ...
      & 					& 9 (beta): \hyperlink{Shepherd2}{Shepherd re-parameterization}: 3 parameters: ln(R0), steepness, and shape parameter, $c$; and\\
      & 					& 10 (beta): \hyperlink{Ricker2}{Ricker re-parameterization}: 3 parameters: ln(R0), steepness, and Ricker power, $\gamma$. This stock recruitment relationship is was added to version 3.30.11 and is in beta mode.\Bstrut\\
    \hline
...
\hypertarget{Shepherd2}{The} Shepherd stock recruit curve re-parameterized version. More details can be found in \citet{punt_extending_2017}.
	\begin{equation}
...
\begin{itemize}
	...
	\item Option 3: no additional parameters are read and all fish are assumed discarded and dead. If the value is 4, then the program will read 7 retention parameters (for dome-shaped retention) and 4 discard mortality parameters.  
...
\myparagraph{Pattern 23 (size) and 24 (size) - Double Normal Selectivity}	
	\begin{itemize}
...
Code for implementing cubic spline selectivity within SS can be found in \hyperlink{CubicSpline}{the appendix} on page \pageref{sec:codeappendix}.
...
\begin{longtable}{p{1cm} p{1cm} p{1cm}  p{1.5cm}  p{2.9cm}  p{1.5cm}  p{4.5cm} }
	...
	\multicolumn{3}{l}{}  &  \multicolumn{4}{l}{0 = No tagging data; and} \\
	...
	-10 & 10 & 9 & 9 & \multicolumn{1}{c}{...} & 0 & \#TG loss chronic3\\
	  1 & 10 & 2 & 2 & \multicolumn{1}{c}{...} & 0 & \#TG loss over dispersion1\\
	  1 & 10 & 2 & 2 & \multicolumn{1}{c}{...} & 0 & \#TG loss over dispersion2\\
	  1 & 10 & 2 & 2 & \multicolumn{1}{c}{...} & 0 & \#TG loss over dispersion3\\
