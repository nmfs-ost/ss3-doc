...
\begin{longtable}{p{1cm} p{3cm} p{11cm}}
	...
	3 & Spawner-            & The options are: \Tstrut\\
	  ...
	  &                     & 5: \hyperlink{Hockey}{Hockey stick}:3 parameters: ln(R0), steepness, and $R_{\text{min}}$) for ln(R0), fraction of virgin \gls{ssb} at which inflection occurs, and the R level at SSB = 0.0; \\
...
Steepness should be used in the equilibrium calculation whenever you believe that the initial equilibrium catch was large enough to have reduced expected recruitment below R0. Note that when using this option, the initial equilibrium catch cannot be greater than \gls{msy}. SS3 uses the identical code for initial equilibrium catch and for \gls{msy} calculation, so it is axiomatic that \gls{msy} will be $<=$ initial equilibrium catch. So, SS3 will estimate a R0 large enough to make \gls{msy} < initial equilibrium catch. Alternatively, you can elect to not use this option and instead add many years to the beginning of the time series with that same level of initial catch. In this case, it is not equilibrium, so the catch can reduce the population below B\textsubscript{MSY}.
...
\begin{longtable}{p{1cm} p{3cm} p{11cm}}
	...
	\multicolumn{2}{l}{1 1980 1 0.20 0.05 4} & Fleet, year, season, $F$, \gls{se}, phase - these values override the catch \gls{se} values in the data file and the overall starting $F$ value and phase read just above. \Bstrut\\
	\hline
...
\begin{itemize}
	\item 0 = skip
	\item 1 = $(1-SPR)/(1-SPR_{TGT})$
	\item 2 = $(1-SPR)/(1-SPR_{MSY})$
	\item 3 = $(1-SPR)/(1-SPR_{Btarget})$
	\item 4 = raw \gls{spr}
\end{itemize}
...
\begin{enumerate}
	...
	\begin{enumerate}[(a)]
		...
		\begin{itemize}
			...
			\item 1 = only R0 active in phase 1. Then finish with no other parameters becoming active; useful for data-limited draws of other fixed parameters. Essentially, this option allows the model to mimic \gls{dbsra}; and
...
\begin{longtable}{p{2cm} p{3cm} p{10cm}}
	...
	22 \Tstrut & 4 & \hyperlink{DoubleNormalPlateau}{Double normal selectivity}; similar to \gls{casal}. \\
...
If the \gls{cv} for size-at-age is being estimated and the model contains mean size-at-age data, then the flag for inclusion of the + ln(stdev) term in the likelihood must be included. Otherwise, the model will always get a better fit to the mean size-at-age data by increasing the parameter for \gls{cv} of size-at-age.
...
\begin{itemize}
	...
	\begin{enumerate}
		\item 0 for not used, 1 to report \gls{ssb} for dynamic B0, 2 to report SSB and recruitment
	\end{enumerate}
