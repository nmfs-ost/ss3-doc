...
A general model for age- and sex-specific natural mortality expands a model developed by Maunder et al. (2009) and Maunder (2011), and is based on the following five assumptions:
\begin{enumerate}
  ...
  \item maturity follows the logistic curve;
...
\end{enumerate}
The model is based on combining Lorenzen’s (2000) observation that M is inversely proportional to the length for young fish and Lehodey et al.’s (2008) logistic model for older fish.
\begin{equation}
M_{s,a} = M_{juv}\frac{L_{s,a}}{L_{mat}}^{\lambda} + \frac{M_{mat,s}-M_{juv}\frac{}L_{s,a}{L_{mat*}}^\lambda}{1+e^{\Beta_s(L_{s,a}- L_{50,s}}}
\end{equation}

with the defaults $\lambda = -1.5$ from Gulland(1987), $\Beta$ and $L_{50}$ from the maturity curve, $M_{mat,s} = 4.118K_{s}^{0.73}L_{inf,s}^{-0.33}$as in Then et al. 2015, and $M_{juv} = 3W_{mat}^{-0.288}$ from Lorenzen (1996), L_{mat} and W_{mat}, are the length and weight of a fish when they first become mature for either sex (could be set at the minimum length over both sexes when 5 percent of the fish are mature) or some other convenient corresponding length and weight of a fish before it becomes mature.

For suboption 1, the maturity values are taken from the maturity relationship, which must use option 1. For suboption 3, the maturity values are specified in parameter lines by the user.
...
\begin{longtable}{p{0.5cm} p{2cm} p{12.75cm}}
	...
	&  & 1 = Requires 4 long parameter lines per sex*growth pattern using age_maturity. Must be used with maturity option 1; \\
	&  & 2 = reserved for future option; \\
	&  & 3 = Requires 6 long parameter lines per sex*growth pattern\Bstrut\\
	\hline
\end{longtable}
