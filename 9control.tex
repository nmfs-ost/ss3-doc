...
\begin{description}
	...
	\begin{itemize}
		\item Recruitment happens in specified settlement events (growth\_pattern, Month, Area);
...
\begin{center}
	\begin{longtable}{p{2cm} p{3cm} p{10cm}}
		...
		17 & nages + 1 & Each age as random walk from previous age.  For all ages in the population beginning with Amin = 1 for the fishery and 2 for the survey, there is a corresponding set of selectivity parameters for each fleet, $p_a$. \\
...
\begin{center}
	\begin{longtable}{p{1cm} p{1cm} p{1cm} p{1.25cm} p{1.25cm} p{1.25cm} p{1.2cm} p{1.2cm} p{1cm} p{1cm} p{1cm}}
		...
		\hline
		\multicolumn{2}{l}{1} &  \multicolumn{5}{l}{Two-dimensional auto-regressive selectivity} & \multicolumn{4}{l} {0 = not used}\\
...
\begin{center}
	\begin{longtable}{p{2cm} p{13cm} }
		\hline
		\multicolumn{2}{l}{End of Control File Input}\\
		\bfseries{999} & \#End of file\\
		\hline
	\end{longtable}
\end{center}
...
The table below shows the link specifications in SS v.3.24 and the corresponding link specification in SS v.3.30.  Take special note of the environmental linkage specification where two bits of information are coded into one number.  The new specification has the environmental link function denoted by the first of three numbers with the environmental index to use specified by two additional numbers. (e.g. environmental link specification of 204 is parsed by SS to use link type 2 using environmental variable 4).
...
\begin{itemize}
	\item Environmental Link and  Variance - Element 8 in parameter setup
		\begin{itemize}
			...
			\item Link = 1: Exponential environmental link ($P'=P*e^{L1*env}$)
			\item Link = 2: Linear environmental link ($P'=P+L1*env$)
			\item Link = 3: Reserved for density dependence
			\item Link = 4: Logistic MGparm environmental link ($P'=P*\frac{2}{1+e^-L1*(env-L2)}$)
		\end{itemize}
