...
A general model for age- and sex-specific natural mortality expands a model developed by \citet{maunder2010bigeye} and \citet{maunder2011M} and is based on the following five assumptions:

\begin{enumerate}
  \item M for younger fish is due mainly to processes (e.g., predation) that are functions of the size of the individuals;
  \item M increases after individuals become reproductively mature;
  \item maturity follows a logistic curve;
  \item M due to reproduction may differ by sex, but juvenile M is independent of sex; and 
  \item M due to senescence over and above that caused by reproduction is either small or occurs at an age for which there are few fish alive, so it is not influential. 
\end{enumerate}
The model is based on combining the observation from \citet{lorenzen2000allometry} that M is inversely proportional to the length for young fish and the logistic model from \citet{lehodey2008spatial} for older fish.
\begin{equation}
M_{s,a} = M_{juv}\frac{L_{s,a}}{L_{mat*}}^{\lambda} + 
\frac{M_{mat,s}-M_{juv}\frac{L_{s,a}}{L_{mat*}}^{\lambda}}{1+e^{\beta_s(L_{s,a}- L_{50,s})}}
\end{equation}

with the defaults $\lambda = -1.5$ from \citet{gulland1987natural}, $\beta$ and $L_{50}$ from the maturity curve, $M_{mat,s}=\frac{5.4}{t_{max,s}}$ from Hamel (submitted) if $t_{max}$ is available, otherwise $M_{mat,s} = 4.118K_{s}^{0.73}L_{inf,s}^{-0.33}$ as in \citet{then2015evaluating} $M_{juv} = 3W_{mat}^{-0.288}$ from \citet{lorenzen1996relationship}. $L_{mat}$ and $W_{mat}$, are the length and weight of a fish when it first becomes mature for either sex (could be set at the minimum length over both sexes when 5\% of the fish are mature) or some other convenient corresponding length and weight of a fish before it becomes mature.

For suboption 1, $L_{50}$ and and slope paramters are taken from the maturity relationship within SS, which must use option 1. For suboption 3, the $L_{50}$ and slope paramters are specified in long parameter lines by the user.
...
The exponential logistic included within SS is based on the exponential logistic selectivity detailed by \citet{thompson_confounding_1994}, however, the parameterization within SS differs.  Explorations using this selectivity form has shown that the estimation of p1 can be highly unstable.  Users are strongly encourage to use the double normal selectivity rather than the exponential logistic selectivity.
	\begin{itemize}
...
An experimental feature added within SS v.3.30.03.02. Earlier versions do not have this feature and hence this input is not expected.  This features allows for a matrix of auto-correlation selectivity deviations by age or length and time as described in \citet{xu_new_2019}. Unlike other options for time-varying selectivity in SS, these deviations are not in the selectivity parameters themselves, but exponential multipliers on whatever baseline selectivity has been chosen.  
...
\begin{longtable}{p{1.1cm} p{1.4cm} p{1.2cm} p{1.2cm} p{1.3cm} p{1.6cm} p{1.4cm} p{1.4cm} p{1.4cm}}
	...
	\multicolumn{3}{l}{ } & \multicolumn{6}{l}{natural mortality, and dynamic B0}\Bstrut\\
	\hline
...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item number of ages for reporting, list entered below (note - each sex will be reported).
	\end{enumerate}
	\item 2 additional values related to dynamic B0:
	\begin{enumerate}
		\item 1 to report (or 0 for not used), and
		\item any value (placeholder for future options.
	\end{enumerate}
\end{itemize}
