...
There are two approaches for fishing mortality. One provides a mid-season harvest rate using Pope’s approximation, the other provides a season-long F rate using the Baranov catch equation. Pope’s harvest rate is implemented as F\_method = 1. The F approach has had more development and is now best implemented using F\_method = 4 which is a superset of F\_Methods 2 and 3.

Within the F approach, there are two ways to represent the F values. One defines each fleet and year/season’s F to be a model parameter, $F_{par}$. The other, termed the hybrid F method, sets each of these F values to be a model factor, $F_{hyb}$, that is internally tuned to maintain a very close match to the observed retained catch. In both cases, the F values are used in the same Baranov catch equation and a catch log-likelihood is calculated based on the observed retained catch and the catch’s se. The $F_{hyb}$ approach works by:
\begin{enumerate}[a.]
	\item Applying the Pope’s method to get a harvest rate for each fleet in the current season;
	\item Converting that harvest rate to the equivalent F. This is exact with one fleet and approximate with multiple fleets;
	\item Adjusting those $F_{hyb}$ values over a fixed number of iterations (2-4) using the ratio of observed to calculated catch for each fleet;
	\item Proceeding with those $F_{hyb}$ values into subsequent model steps just as if those values had come from model parameters.
\end{enumerate}

F\_Method 3 implements the $F_{hyb}$ approach for all fishing fleets. F\_Method 2 implements the $F_{par}$ approach for all fishing fleets, but allows for starting values of these parameters to be calculated in early model phases using the hybrid approach, then copying those $F_{hyb}$ values into the $F_{par}$ values beginning in a specified phase. F\_method 4 extends this flexibility by allowing the transition from hybrid to parameter be done at a fleet-specific phase and includes the ability to maintain some fleets at hybrid throughout the model run.

Some notes:
\begin{itemize}
	\item At high F and/or with many fleets, the hybrid approach may not get to an exact match to the observed catch.
	\item At moderate F levels, hybrid should get a near exact match to the catch even if the catch se is high.
	item At high F, the problem becomes quite computationally stiff for Pope’s approximation and the hybrid method so convergence in ADMB may slow due to more sensitive gradients in the log likelihood with regard to other model parameters. Whereas, ADMB adjusts the $F_{par}$ values slowly over the entire sequence of iterations, so it can converge with fewer iterations.
	\item The hybrid approach performs best when the catch is known with high precision (s.e.  < ~ 0.05) and the total F across all fleets is not much greater than M. The F as parameter approach works best when total F is high; also where a fleet has both retained catch and discarded catch, because the F is influenced by both. 
	\item With F method 4, fleets with low F can $F_{hyb}$ to the end and high F fleets can switch to $F_{par}$.
	\item It is always advisable to allow the model to start with good starting values for parameters. This can be done by specifying a later phase (>1) under the conditional input for F method = 2 where early phases will use the hybrid method, then switch to F as parameter in later phases and transfer the hybrid F values to the parameter initial values. Same advice for using the F method = 4. However, bycatch fleets do not have retained catch, so cannot use the hybrid method. They must use $F_{par}$ with user-specified starting value starting in phase 1.
...
% There are four methods available for calculation of fishing mortality (F): 1) Pope's approximation, 2) Baranov's continuous F with each F as a model parameter, 3) a hybrid F method, and 4) a fleet-specific parameter hybrid F approach (introduced in v.3.30.18).
