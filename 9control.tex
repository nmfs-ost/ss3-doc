...
\begin{itemize}
	...
	\item \textbf{Advisable Estimation Practice:} Because a penalized likelihood approach like that in SS3 may have a negative bias when directly estimating the standard deviation parameter ($s$), it is advisable to profile on fixed values for $s$ to find a range that yields good results. A good starting criteria for selection is a value for $s$ such that the sum of the variance among the deviations ($\text{var}(\mathbf{d_y})$) and the mean variance of each deviation ($\text{mean}(\text{var}(\mathbf{d_y}))$) is close to 1.
