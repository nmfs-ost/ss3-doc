...
\subsubsection{Time Blocks and Time-varying Parameter Controls}
...
\begin{longtable}{p{1cm} p{1cm} p{1cm}  p{1.5cm}  p{2.9cm}  p{1.5cm}  p{4.5cm} }
	...
	\multicolumn{3}{l}{}  &  \multicolumn{4}{l}{ here will auto-generate the tag parameter section in the control.ss\_new file.; and}\\
...
A shortcoming, present in SS versions prior to 3.30.14, was identified in the recapture calculations when using Pope's F Method and multiple seasons. The internal calculations were corrected in version 3.30.14. Now the Z-at-age is applied internally for calculations of fishing pressure on the population when using  the Pope calculations.

\myparagraph{Mirroring of Tagging Parameters}
In version 3.30.14 the ability to mirror the tagging parameters from another tag group or fleet.  With this approach, the user can get away with just one parameter value for each of the five parameter types and everything else mirrored (but still needing a line of input in the parameter file). This new feature is evoked through the phase input in the tagging parameter section.  The options now are:
\begin{itemize}
	\item No mirroring among tag groups or fleets: phase > -1000,
	\item Mirror a lower (already specified) tag group or fleet: phase = -1000 and set parameter same as next lower Tag Group or fleet,
	\item Mirror a lower (already specified) tag group of fleet X: phase = -100x set parameter same as Tag Group or fleet where x is the fleet number to mirror to (i.e., if you would like to mirror fleet 1 then the phase should -1001).
\end{itemize}
