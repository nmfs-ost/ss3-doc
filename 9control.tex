...
\myparagraph{Mean sizea-at-maximum age}
...
\hypertarget{Ricker2}{The} Ricker stock recruit curve re-parameterized version (More details can be found in Punt and Cope, in press)
	\begin{equation}
...
\begin{itemize}
	\item Environmental Link and  Variance - Element 8 in parameter setup
		\begin{itemize}
		%	\item New approach to the coding of the link type (MGparm\_envtype) and environmental variable (MGparm\_envuse) being linked %to: %Parsing: Link = value / 100; Env\_var = value - Link*100
		%	\begin{itemize}
		%		\item 100 - 199  to indicate one parameter multiplicative link, with the environmental variable found by subtracting %100.  For example, a value of 102 would create a multiplicative linkage to environmental variable "2". %Link = 1: %Exponential environmental link ($P'=P*e^{L1*env}$)
		%		\item 200 - 299  for one parameter additive linkage %Link = 2: Linear environmental link ($P'=P+L1*env$)
		%		\item 400 - 499  for  2 parameter logistic function %Link = 3: Reserved for density dependence
		%		\item etc. %Link = 4: Logistic MGparm environmental link ($P'=P*\frac{2}{1+e^-L1*(env-L2)}$)
		%	\end{itemize}
