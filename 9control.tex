...
\begin{equation}
M_{a,s} = M_{juv}\frac{L_{a,s}}{L_{mat*}}^{\lambda} + 
\frac{M_{mat,s}-M_{juv,s}\frac{L_{a,s}}{L_{mat*}}^{\lambda}}{1+e^{\beta_s(L_{a,s}- L_{50,s})}},
\end{equation}

where $M_{juv}$ (juvenile natural mortality), $\lambda$ (power), $L_{mat*}$ ( first mature length of fish), and $M_{mat,s}$ (the mature instantaneous natural mortality rate by sex, are user inputs in long parameter lines. For suboption 1, $L_{50}$ and and $\beta$ (slope) parameters taken from the maturity relationship within SS, which must use maturity-fecundity option 1. For suboption 3, the $L_{50,s}$  (the length at which 50\% of fish are mature) and $\beta$ (slope) paramters are specified in long parameter lines by the user.
