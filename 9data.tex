...
Seasons
	 ...
	 \end{itemize}
\hypertarget{SubSeas}{Subseasons and Timing of events in SS v3.30}
       \\ SS3.24 and all earlier versions effectively had two subseasons per season because the age-length-key (ALK) for each observation used the mid-season mean length-at-age and spawning occurred at the beginning of a specified season.  Subseasons in SS3.30 provide more precision in the timing of events.
	\begin{itemize}
...
\begin{description}
	\item[Length Composition Data Stucture:]\
...
\begin{center}
	\begin{tabular}{p{2cm} p{2cm} p{2cm} p{2cm} p{2cm} p{2cm} p{2cm}}
		\multicolumn{6}{l}{Specify bin compression and error structure for age composition data for each fleet:}\\
		\hline
...
\begin{center}
	\begin{tabular}{p{2cm} p{2cm} p{2cm} p{2cm} p{2cm} p{2cm} p{2cm}}
		\multicolumn{6}{l}{Specify method by which length bin range for age obs will be interpreted:}\\
		\hline
