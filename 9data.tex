...
\begin{center}
	\begin{tabular}{p{4cm} p{12cm}}
			...
			& \\
			\hline
			\hypertarget{SubSeas}{2} & \multirow{1}{1cm}[-0.1cm]{\parbox{12cm}{The number of subseasons.  Entry must be even and the minimum value is 2. This is for the purpose of finer temporal granularity in calculating and using the length-at-age.}}\\
			& \\
			& \\
			& \\
			\hline
...
\subsubsection{Catch}
\hypertarget{CatchFormat}{The} new format for version 3.30 for a 2 season model with 2 fisheries looks like the table below.  The example is sorted by fleet, but the sort order does not matter.  In data.ss\textunderscore new, the sort order is fleet, year, season.
