...
As of ADMB 12.3, a new command called "-hess_step" is available to use and documented in the \hyperlink{hess_step}{Using -hess_step to do single Newton steps using the inverse Hessian}
...
\hypertarget{hess_step}{}
\subsubsection{Using -hess_step to do single Newton steps using the inverse Hessian}
...
The feature is used by optimizing normally, and then from the command line running -hess_step for defaults (recommended), -hess_step N, or -hess_step_tol eps where N and eps are the maximum number of steps to take and the tolerance (i.e., a very small number like 1e-10) after which to stop.
