...
The specification of options for benchmark (reference points) and forecasts is contained in the mandatory input file named \texttt{forecast.ss}. The benchmark code in SS3 is executed after the model has converged its search for the maximum log-likelihood, or after each call to MCEVAL when doing MCMC. The benchmark code searches for the fleet-specific $F$ values that will achieve a user-selected condition in equilibrium. For example, it can search for the $F$s that would achieve maximum sustainable catch, or search for the $F$s that would reduce the spawning stock biomass to 40\% of the unfished spawning stock biomass. After the benchmark calculations are complete, SS3 proceeds to conduct the forecast. The forecast is basically a continuation of the time series using a user-selected fishing strategy rather than the time series approach which finds $F$s that match the input catch. The forecast has controls for a control rule that adjusts $F$ as a function of SSB, for allocation of catch among fleets, for control of recruitment and numerous other features. Both the benchmark and forecast sections have options to specify a range of years from the time series from which quantities like body weight-at-age, selectivity, and other factors are averaged for use in benchmark and/or forecast. A particular aspect of the forecast is that it is repeated in up to 3 sequential passes with different conditions regarding which aspects of the forecast are applied. See \hyperref[sec:forecast]{Forecast Module: Benchmark and Forecasting Calculations} for additional details.
...
\subsection[Forecast File Options (\texttt{forecast.ss})]{\protect\hyperlink{fore-specify}{Forecast File Options (\texttt{forecast.ss})}}
  ...
  \hline
  \multicolumn{3}{l}{\textbf{Next are specifications for the forecast.}} \\
  ...
  % \pagebreak
  \hline
  \multicolumn{3}{l}{There are 2 options for entering \hypertarget{FcastYears}{Forecast Years}: Option 1 is a legacy option and option 2 became available in v3.30.23. Option 2 is recommended because it provides better control of year averaging and blocks, random effects, and other time-varying parameter changes can now operate on forecast years to provide year-specific changes during forecast years.} \Tstrut\Bstrut\\
  
  Option 1: & & \Tstrut\\

  \pagebreak
  0 0 0 0 0 0 & Enter 6 Forecast Year Values & \multirow{1}{1cm}[-0.15cm]{\parbox{12cm}{A line with 6 values: beginning and ending years for selectivity, relative $F$s, and recruitment distribution. These are used to create means over the specified range of years. Values can be entered as the actual year, -999 for start year, or values of 0 or -integer to be relative endyr. It is important to note:}} \Tstrut\Bstrut\\
   & & \Bstrut\\
   & & \Bstrut\\
  ...
  Option 2: & \multicolumn{2}{l}{\multirow{1}{1cm}[-0.15cm]{\parbox{18.5cm}{To use the new approach, enter -12345 in the location of the first forecast year, omit the 2nd thru 6th forecast years and omit the entry of the \hyperlink{FcastSelectivity}{Forecast Selectivity Option}.}}} \Tstrut\Bstrut\\
  & & \\
  ...
  \hline
  % \pagebreak
  0.75 \Tstrut & Control Rule Buffer (multiplier between 0-1.0 or -1) & \multirow{1}{1cm}[-0.25cm]{\parbox{12cm}{Control rule catch or $F_{TARGET}$ as a fraction of selected catch or $F_{MSY}$ proxy. The buffer will be applied to reduce catch from the estimated OFL. The buffer value is a value between 0-1.0 where a value of 1.0 would set catch equal to the OFL. As example if the buffer is applied to catch (Control Rule option 3 or 4 above) the catch will equal the buffer times the OFL. Alternatively a value of -1 will allow the user to input a forecast year specific control rule fraction (added in v.3.30.13).}} \Bstrut\\ 
    & & \Bstrut\\
    & & \Bstrut\\
    & & \Bstrut\\
    & & \Bstrut\\
