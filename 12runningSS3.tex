...
\href{https://github.com/shcaba/SS-DL-tool}{The Stock Assessment Continuum Tool} (previously known as the Stock Synthesis Data-limited Tool) provides an Shiny application interface upload catch time series, age composition, length composition, and abundance index data and then define model options within the Shiny application, rather than in the Stock Synthesis input files. While the tool initially was created to assist in implementing data-limited assessment methods, the tool is now able to implement a variety of types of models.
