...
An additional command has been added that allows users to specify the name of the .par file. Prior to v.3.30.22.1, the default exe name was `ss' and the default .par file name was ss.par. The code now produces a ss3.par file by default instead of a ss.par file. If you would still like to produce a ss.par file or a .par file with a different name, for example `ss4you.par', you will need to add ``modelname ss4you'' after the executable name such as:
