...
To run SS3 without estimation use: `ss3 -stopph 0`. This will speed up your run by not optimizing. Often `-stopph 0` is used with the `-nohess` option to speed up the run even more. To run SS3 with no estimation in v.3.30.15 and earlier, change the max phase in the starter.ss file to 0 and run the exe with the `–nohess` option. Prior to version v.3.30.22.1, the par file name was ss.par, but now the default name is ss3.par. To have SS3 create a par file with a different name, such as ss4you.par, use: `ss3 modelname ss4you`.
